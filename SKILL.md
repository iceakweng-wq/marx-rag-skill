...
**搜索 agent** → 语义检索 + 翻页扩展 → 只返回 `(卷次, 页码)` 列表
**主 agent** → 汇总、去重、合并页码范围 → 传给评审 agent
**评审 agent** → 判断是否足够 → 输出结构化摘要
...
- 收到主 agent 传来的汇总页码范围
- 用 `python scripts/search.py --page {卷次} {页码1} {页码2} ...` 按需拉取原文阅读
- **通读全部原文**后判断：
  - 内容是否足够回答用户问题
  - 覆盖范围是否多样（不同文本、不同时期的论述）
  - 是否需要补充搜索其他关键词
- **如果不够**：告诉主 agent 还需要搜索哪些关键词，主 agent 启动新的搜索 agent，更新汇总后再次传给评审 agent
- **如果够了**：输出**结构化摘要**（格式见下文），返回给主 agent
