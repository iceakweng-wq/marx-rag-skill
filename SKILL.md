...
### 查询任务
当用户提出搜索、查询、查找、检索某概念、某理论、某段原文，或问"马克思/恩格斯对XX怎么看""XX在原著中怎么论述的"等问题时，执行查询任务。并将结果用markdown渲染。

读取 `sub_agent/rag_agent.md` 中的要求执行。

### 阅读原文
当用户说"看原文""展示原文""读一下原文""打开原文"等时，执行阅读原文任务。

读取 `sub_agent/read_txt.md` 中的要求执行。

### 都不是
如果用户的问题既不是查询也不是阅读原文（如闲聊、无关话题），不调用此技能相关工具，正常对话即可。
